...
const answerFinancialQuestionsFlow = ai.defineFlow(
  ...
  async (input: AnswerFinancialQuestionsInput): Promise<AnswerFinancialQuestionsOutput> => {
    ...
    (async () => {
      try {
        console.log(`[answerFinancialQuestionsFlow - Summarization] Attempting background summarization for session: ${sessionId}, query: ${queryId}`);
        ...
        if (summaryOutput && summaryOutput.summaryText && summaryOutput.summaryText.trim() !== "") {
          console.log(`[answerFinancialQuestionsFlow - Summarization] Summary generated by flow: "${summaryOutput.summaryText.substring(0, 100)}..." Attempting to save to Firestore for session: ${sessionId}, queryId: ${queryId}`);
          await addSummaryToSession(sessionId, {
            summary_text: summaryOutput.summaryText,
            query_id: queryId,
          });
          console.log(`[answerFinancialQuestionsFlow - Summarization] Successfully generated and saved summary to Firestore for session: ${sessionId}`);
        } else {
        ...
        console.error(
          `[answerFinancialQuestionsFlow - Summarization] Error during background summarization for session ${sessionId}:`,
